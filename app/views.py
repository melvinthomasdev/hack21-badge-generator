...
# print(root)
template_url = root + staticfiles_storage.url(r'img\aurd.png')
        ...
        if form.is_valid():
            if request.FILES:
                ...
                if thumbimg.width > thumbimg.height:
                    reqht = 1232
                    reqwd = int((thumbimg.width/thumbimg.height)*1232)
                if thumbimg.width < thumbimg.height:
                    reqwd = 1278
                    reqht = int((thumbimg.height/thumbimg.width)*1278)
                if thumbimg.width == thumbimg.height:
                    reqwd = reqht = 209
                ...
                thumbimg = thumbimg.resize((1278, 1232)) # widht, height
                ...
                response['Content-Disposition'] = 'attachment; filename=%s.png' % "Arduino Week Badge"
